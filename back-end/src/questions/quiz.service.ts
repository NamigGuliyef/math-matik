...
export class QuizService {
  ...
  async completeStage(
    ...
    log(
      `Completing stage: User ${userId}, Grade ${grade}, LevelCap ${levelCap}, Stage ${stage}`,
    );

    // Track Mission Progress
    await this.missionsService.trackProgress(
      userId,
      MissionType.STAGE_COMPLETE,
      1,
    );
    ...
    const stageId = `${grade}:${levelCap.toLowerCase()}:${stage}`;
    ...
    log(
      `Parsed Level: ${levelNum}, Target Item Level: ${targetItemLevel}, Already Completed: ${isAlreadyCompleted} `,
    );
    ...
    const suitableItems = await this.itemModel
      .find({ level: targetItemLevel })
      .exec();

    log(
      `Found ${suitableItems.length} suitable items for level ${targetItemLevel}`,
    );

    if (suitableItems.length === 0) {
      log(
        `WARNING: No items found for level ${targetItemLevel} in the database.`,
      );
    ...
    const userInventory = await this.inventoryModel
      ...
      .exec();
    const ownedItemIds = userInventory
      .map((i) => i.itemId?.toString())
      .filter((id) => !!id);

    log(
      `User owns ${ownedItemIds.length} items.Checking for variety in level ${targetItemLevel}.`,
    );

    // Prioritize items user doesn't own yet
    let pool = suitableItems.filter(
      (item) => !ownedItemIds.includes(item._id.toString()),
    );

    // If user owns all items of this level, use the full pool
    if (pool.length === 0) {
      log(
        `User already owns all suitable items for level ${targetItemLevel}.Using full pool.`,
      );
      pool = suitableItems;
    } else {
      log(
        `Found ${pool.length} items user doesn't own yet. Picking from this subset.`,
      );
    }
    ...
    log(
      `Progress updated: ${currentProgress}% -> ${newProgress}%. Item Awarded: ${itemAwarded}`,
    );
