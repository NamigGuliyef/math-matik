...
export class ChatService {
    ...
    async postMessage(userId: string, message: string) {
        ...
        if (message.length > 200) { // Increased for tagging
        ...
        if (user.totalAnswered < 20) {
            throw new BadRequestException('Mesaj yazmaq üçün minimum 20 sual tamamlamalısınız');
        }

        const now = Date.now();
        const lastTime = this.lastMessageTime.get(userId) || 0;
        if (now - lastTime < 5000) {
            throw new BadRequestException('Hər 5 saniyədə yalnız bir mesaj göndərə bilərsiniz');
        }
