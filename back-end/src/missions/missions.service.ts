...
export class MissionsService {
  ...
  constructor(
    ...
    private progressModel: Model<UserMissionProgress>,
  ...
  async trackProgress(
    ...
    const activeMissions = await this.missionModel
    ...
    const activeAchievements = await this.achievementModel
      .find({ type, isActive: true })
      .exec();
  ...
  private async updateProgress(
    userId: string,
    targetId: string,
    targetType: 'mission' | 'achievement',
    increment: number,
  ) {
  ...
  async openChest(userId: string) {
    ...
    const result: any = { type: reward.type };
